...
function loadDataFromJson(filePath: string): any {
...
function flattenData(
  rawData: Record<string, Record<string, Entry[]>>
): Entry[] {
  const flatArray: Entry[] = [];
...
function generateHtmlTableGrouped(data: Entry[]): string {
  // Hypomyces speciesでグループ化
  const grouped = data.reduce<Record<string, Entry[]>>((acc, entry) => {
    if (!acc[entry.hypomyces_species]) acc[entry.hypomyces_species] = [];
